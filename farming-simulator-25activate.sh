#!/bin/bash
# Activates Farming Simulator 25 by driving the launcher's product key dialog under Proton.
#
...
PROFILE="$PROFILE/My Games/FarmingSimulator2025"
ACTIVATION="$PROFILE/AHC_63805.dat"

if [[ -f "$ACTIVATION" ]]; then
    echo "Farming Simulator 25 is already activated for this instance"
    exit 0
fi
...
LAUNCHER="$GAME_DIR/FarmingSimulator2025.exe"
...
cleanup() {
    pkill -f 'FarmingSimulator2025Game.exe' >/dev/null 2>&1
    pkill -f 'FarmingSimulator2025.exe' >/dev/null 2>&1
    [[ -n "${XVFB_PID:-}" ]] && kill "$XVFB_PID" >/dev/null 2>&1
    return 0
}
trap cleanup EXIT

echo "Activating Farming Simulator 25..."
"$PROTON" runinprefix "$LAUNCHER" >/dev/null 2>&1 &

# The dialog is titled "Product Key for Farming Simulator 25"; match loosely in case it is localised.
WID=""
deadline=$(( SECONDS + DIALOG_TIMEOUT ))
while (( SECONDS < deadline )); do
    sleep 2
    # Farming Simulator 25 uses the GIANTS Launcher, whose window is titled with the product and build
    # ("FarmingSimulator2025    v47341") rather than naming the key prompt, so match either form.
    WID=$(xdotool search --onlyvisible --name 'Product Key|FarmingSimulator2025' 2>/dev/null | head -1)
    [[ -n "$WID" ]] && break
done
