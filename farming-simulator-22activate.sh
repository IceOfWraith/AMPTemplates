#!/bin/bash
# Activates Farming Simulator 22 by driving the launcher's product key dialog under Proton.
#
...
PROFILE="$PROFILE/My Games/FarmingSimulator2022"
ACTIVATION="$PROFILE/AHC_63805.dat"

if [[ -f "$ACTIVATION" ]]; then
    echo "Farming Simulator 22 is already activated for this instance"
    exit 0
fi

KEY="${KEY// /}"
if [[ -z "$KEY" ]]; then
    echo "ERROR: Farming Simulator 22 is not activated for this instance and no product key is set."
    echo "       Set the Product Key setting on this instance, then start it again."
    exit 1
fi

LAUNCHER="$GAME_DIR/FarmingSimulator2022.exe"
...
cleanup() {
    pkill -f 'FarmingSimulator2022Game.exe' >/dev/null 2>&1
    pkill -f 'FarmingSimulator2022.exe' >/dev/null 2>&1
    [[ -n "${XVFB_PID:-}" ]] && kill "$XVFB_PID" >/dev/null 2>&1
    return 0
}
trap cleanup EXIT

echo "Activating Farming Simulator 22..."
"$PROTON" runinprefix "$LAUNCHER" >/dev/null 2>&1 &

# The dialog is titled "Product Key for Farming Simulator 22"; match loosely in case it is localised.
WID=""
deadline=$(( SECONDS + DIALOG_TIMEOUT ))
while (( SECONDS < deadline )); do
    sleep 2
    # Older titles title this window "Product Key for ..."; newer ones use the GIANTS Launcher, whose window
    # is titled with the product and build instead. Match either so the same script covers both.
    WID=$(xdotool search --onlyvisible --name 'Product Key|FarmingSimulator2022' 2>/dev/null | head -1)
    [[ -n "$WID" ]] && break
done

if [[ -z "$WID" ]]; then
    [[ -f "$ACTIVATION" ]] && { echo "Farming Simulator 22 is already activated for this instance"; exit 0; }
